...
On an installation with one organization, Nakama moves the existing WhatsApp connection into that organization when the server restarts, preserving the linked device and settings. If you started the bridge manually, stop it before restarting the server.

On an installation with several organizations, the existing shared bridge stays available. Each organization can configure its own number separately; Nakama does not guess which organization owns the old connection.
