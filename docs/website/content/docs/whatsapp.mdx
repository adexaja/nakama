...
Each organization can connect its own WhatsApp number and choose its own **Reply as** agent. For example, use one organization and number for Well Test reports and another for Finance Track. Starting, stopping, or reconnecting one organization's bridge does not interrupt the other.
...
2. Select the organization, then open **Integrations → WhatsApp**
3. Choose which profile should reply
4. Click **Enable WhatsApp**

### 2. Start the bridge

Use the bridge's **Start** control in **Integrations → WhatsApp**. Repeat these steps in each organization with a different WhatsApp number.
...
| `/org` | Switches organizations only on a legacy shared connection |
...
2. the WhatsApp bridge is running in the selected organization's **Integrations → WhatsApp**
...
When the server restarts, Nakama moves the existing shared WhatsApp connection into the first-created organization, preserving the linked device and settings. This also applies when you have several organizations. If you started the bridge manually, stop it before restarting the server.

Other organizations must connect their own WhatsApp numbers. Nakama does not overwrite an existing WhatsApp connection in the first-created organization or move the shared connection to another organization instead.
