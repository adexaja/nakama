...
function encryptionKey(): Buffer {
  ...
  try {
    raw = parseIniWithSections(readFileSync(getUserConfigPath(), "utf8"))
      .sections.security?.mfa_encryption_key;
  } catch {
    raw = undefined;
  }
  if (!raw) {
    throw new Error("MFA encryption key is not configured in config.ini.");
  }
  const key = Buffer.from(raw, "base64url");
  if (key.length !== 32) {
    throw new Error(
      "MFA encryption key must be a base64url-encoded 32-byte key."
    );
  }
  return key;
}
...
export async function ensureMfaEncryptionKey(): Promise<string> {
  let parsed = { global: {}, sections: {} } as ReturnType<
    typeof parseIniWithSections
  >;
  try {
    parsed = parseIniWithSections(readFileSync(getUserConfigPath(), "utf8"));
  } catch {
    // The config file is created by writeParsedConfigIni below.
  }
  const existing = parsed.sections.security?.mfa_encryption_key;
  if (existing) {
    return existing;
  }
  const key = randomBytes(32).toString("base64url");
  parsed.sections.security ??= {};
  parsed.sections.security.mfa_encryption_key = key;
